            ...
            if(self.costmap.costmap[self.goal_node.map_i][self.goal_node.map_j]) == 0: #depends on how you set the value of obstacle
                ...
                while (hit_obstacle == True): #continue finding a random point that does not collide with obstacle
                    ...
                    while ri >= self.map_width or rj >= self.map_height:
                        ...
                        rj = random.randint(self.start_node.map_j-r,self.start_node.map_j + r) # Let's make a random number!
                        tries +=1
                    while bfsdistance[self.start_node.map_i][rj] >= bfsdistance[self.start_node.map_i+1][self.start_node.map_j+1] and bfsdistance[ri][self.start_node.map_j] >= bfsdistance[self.start_node.map_i+1][self.start_node.map_j+1]:
                        ri = random.randint(self.start_node.map_i-r,self.start_node.map_i + r)
                        rj = random.randint(self.start_node.map_j-r,self.start_node.map_j + r) # Let's make a random number!
                        tries +=1    
                    while ri >= self.map_width or rj >= self.map_height:
                    ...
                    try:
                        if bfsdistance[self.start_node.map_i][rj] >=  bfsdistance[ri][self.start_node.map_j]:
                            rj = self.start_node.map_j
                        else:
                            ri = self.start_node.map_i
                    ...
                    if(hit_obstacle==False):
                        ...
                        a,b = self.map2world(ri,rj)
                        self.robot.state_des.add_destination(x=a,y=b,theta=0)
...
#confirm bias with bfs distance
    ...
    #restrict possible nodes to within map boundary                    
                ...
                if(self.check_vicinity(self.goal_node.map_i,self.goal_node.map_j,ri,rj,50.0)):
            ...
            a,b = self.map2world(self.goal_node.map_i,self.goal_node.map_j)
            self.robot.state_des.add_destination(x=a,y=b,theta=0)
