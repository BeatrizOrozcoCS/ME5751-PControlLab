...
import math
import time
#C:\Users\Orozc\OneDrive\My Documents\CPP\Fall 2022\ME 5751\Lab 1\changyizhe-me5751_labs-47fac0a37daa
...
class P_controller:
    ...
    def __init__(self, robot, logging = True):
        ...
        if(logging == True):
            self.robot.make_headers(['kp','kb','ka','dpos_X','dpos_Y','dpos_theta', 'cpos_X','cpos_Y','cpos_theta', 'alpha', 'beta', 'vix','viy','wi','vr','wr','phi_l','phi_r'])
    ...
    def set_goal_points(self):
        ...
        for i in range (0,turns):
            ...
            self.robot.state_des.add_destination(x=0,y=200,theta=math.pi)    #goal point 1
            self.robot.state_des.add_destination(x=0,y=-200,theta=math.pi) #goal point 2
            self.robot.state_des.add_destination(x=-100,y=0,theta=0) #goal point 2
    ...
    def track_point(self):
        ...
        #calcualte deltaX, deltaY,rho, alpha
        ...
        theta = c_theta
        #dislaplay 
        if (alpha <= math.pi/2 and alpha > -math.pi/2 ): # go forward
            beta = -theta - alpha
            c_v = self.kp*rho
            ...
            beta = -theta- math.atan2(-deltaY,-deltaX)
            c_v = -self.kp*rho
            c_w = self.ka*alpha + self.kb*beta
        ...
        if self.logging == True:
            self.robot.log_data([self.kp,self.kb,self.ka,d_posX,d_posY,d_theta, c_posX,c_posY,c_theta, alpha, beta, c_vix,c_viy,c_wi,c_v,c_w,phi_l,phi_r])
        
        threshold = 3 # threshold constant
        radsthreshold = 0.01
        if abs(rho) <threshold and abs(abs(c_theta)-abs(d_theta))<radsthreshold : #you need to modify the reach way point criteria
